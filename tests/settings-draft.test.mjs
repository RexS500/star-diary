...
const state = () => ({
  ...
  redemptions: [],
  specialRewards: [],
  templates: [{ id: "t1", title: "整理書包", amount: 2, type: "star", sortOrder: 0 }],
...
test("all editable setting families affect the signature", () => {
  ...
  const mutations = [
    ...
    draft => { draft.templates.push({ id: "t2", title: "幫忙", amount: 1, type: "star", sortOrder: 1 }); },
    draft => { draft.templates[0].type = "deduct"; },
    draft => { draft.templates[0].amount = 3; },
    draft => { draft.templates[0].sortOrder = 1; },
