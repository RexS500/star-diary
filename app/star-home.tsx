...
function prepareSettingsForSave(draft:State):State{
    ...
    return {
        ...
        entries:draft.entries.filter(entry=>childIds.has(entry.childId)),
        redemptions:draft.redemptions.filter(redemption=>childIds.has(redemption.childId)),
        dailyTasks:draft.dailyTasks.filter(task=>childIds.has(task.childId)),
        dailyTaskRecords:draft.dailyTaskRecords.filter(record=>childIds.has(record.childId)),
        dailyTaskSettings:Object.fromEntries(Object.entries(draft.dailyTaskSettings).filter(([childId])=>childIds.has(childId))),
    };
}
const fallback: State = { children: [{ id: "c1", name: "小宇", gender: "boy", avatar: "boy", stars: 0 }], entries: [], rewards: [{ id: "r1", icon: "🍦", name: "冰淇淋", cost: 12, stock: 0 }], templates: [{ id: "t1", title: "主動整理書包", amount: 3, type: "star" }], redemptions: [],specialRewards:[],rewardIconLibrary:[],dailyTasks:[],dailyTaskRecords:[],dailyTaskSettings:{} };
...
const entryTimestamp = (entry:Entry) => {const created=entry.createdAt?entryTime(entry.createdAt):Number.NaN;return Number.isFinite(created)?created:entryTime(entry.date)};
...
export default function App() {
    ...
    const [crop,setCrop]=useState<CropTarget|null>(null),[todayKey,setTodayKey]=useState(taipeiDateKey()),[taskBusy,setTaskBusy]=useState(""),[taskConfirm,setTaskConfirm]=useState<DailyTaskRecord|null>(null);
    ...
    const currentSettingsSignature=useMemo(()=>settingsSignature(data,newPasswordDraft),[data,newPasswordDraft]);
    const hasUnsavedChanges=Boolean(savedSettingsSignature)&&currentSettingsSignature!==savedSettingsSignature;
    const showSettingsSaveBar=role==="家長"&&tab==="家庭設定"&&(hasUnsavedChanges||imageUploading||savingSettings);
    ...
    function normalizedClientState(loaded:Partial<State>):State{return {...fallback,...loaded,children:Array.isArray(loaded.children)?loaded.children:fallback.children,entries:Array.isArray(loaded.entries)?loaded.entries:[],rewards:Array.isArray(loaded.rewards)?loaded.rewards:[],templates:Array.isArray(loaded.templates)?loaded.templates:[],redemptions:Array.isArray(loaded.redemptions)?loaded.redemptions:[],specialRewards:Array.isArray(loaded.specialRewards)?loaded.specialRewards:[],rewardIconLibrary:Array.isArray(loaded.rewardIconLibrary)?loaded.rewardIconLibrary:[],dailyTasks:Array.isArray(loaded.dailyTasks)?loaded.dailyTasks:[],dailyTaskRecords:Array.isArray(loaded.dailyTaskRecords)?loaded.dailyTaskRecords:[],dailyTaskSettings:loaded.dailyTaskSettings&&typeof loaded.dailyTaskSettings==="object"?loaded.dailyTaskSettings:{}}}
    function establishSettingsBaseline(next:State){const snapshot=clonePersistedState(next);savedStateRef.current=snapshot;setSavedSettingsSignature(settingsSignature(snapshot,""))}
    function applyServerPayload(result:{state?:Partial<State>;revision?:number;passwordSet?:boolean},options:{replaceSettings?:boolean;establishBaseline?:boolean}={}){const incoming=Number(result.revision);if(Number.isFinite(incoming)&&incoming<revisionRef.current)return false;if(result.state){const next=normalizedClientState(result.state),preserveDraft=settingsDirtyRef.current&&!options.replaceSettings;if(preserveDraft)setData(current=>applyEditableSettings(next,current));else{setData(next);setCid(current=>next.children.some(item=>item.id===current)?current:(next.children[0]?.id||current));if(options.establishBaseline!==false)establishSettingsBaseline(next)}}if(Number.isFinite(incoming))revisionRef.current=incoming;if(typeof result.passwordSet==="boolean")setPasswordSet(result.passwordSet);return true}
    async function reloadState(){try{const response=await fetch(`/api/state?t=${Date.now()}`,{cache:"no-store"}),result=await response.json();if(!response.ok||!result.state)throw new Error(result.error||"無法讀取資料");const applied=applyServerPayload(result);setTodayKey(taipeiDateKey());return applied}catch{return false}}
    async function refreshAnalytics(){const ok=await reloadState();say(ok?"分析資料已刷新":"刷新失敗，請檢查網路後再試");return ok}
    async function rebaseSettingsDraft(draft:State){try{const response=await fetch(`/api/state?t=${Date.now()}`,{cache:"no-store"}),result=await response.json();if(!response.ok||!result.state)return false;const incoming=Number(result.revision);if(Number.isFinite(incoming))revisionRef.current=incoming;if(typeof result.passwordSet==="boolean")setPasswordSet(result.passwordSet);const remote=normalizedClientState(result.state),merged=applyEditableSettings(remote,draft);establishSettingsBaseline(remote);setData(merged);setCid(current=>merged.children.some(item=>item.id===current)?current:(merged.children[0]?.id||current));setTodayKey(taipeiDateKey());return true}catch{return false}}
    async function persist(next: State, newPassword?: string,options:PersistOptions={}) {if(options.optimistic!==false)setData(next);try{const r = await fetch("/api/state", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: "save", state: next, password, newPassword,expectedRevision:revisionRef.current }) }),x=await r.json();if(!r.ok){if(options.preserveDraftOnFailure){if(r.status===409&&options.draftForRebase&&await rebaseSettingsDraft(options.draftForRebase))say("其他裝置已有新資料，未儲存內容已保留，請確認後再次儲存");else say(x.error || "儲存失敗，未儲存內容仍為你保留");return false}await reloadState();say(x.error || "儲存失敗");return false}const applied=applyServerPayload(x,{replaceSettings:true,establishBaseline:true});if(!applied){say("收到較舊的儲存結果，請再試一次");return false}if(newPassword){setPassword(newPassword);setNewPasswordDraft("")}say(options.successMessage||"已儲存所有設定");return true}catch{if(!options.preserveDraftOnFailure)await reloadState();say(options.preserveDraftOnFailure?"儲存失敗，未儲存內容仍為你保留":"儲存失敗，請檢查網路後再試");return false}}
    async function submitPending(action:"child_entry"|"child_redemption",record:Entry|Redeem,next:State){setData(next);try{const r=await fetch("/api/state",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action,record})}),x=await r.json();if(!r.ok){await reloadState();say(x.error||"送出失敗");return false}applyServerPayload(x);say("已送出，等待家長確認");return true}catch{await reloadState();say("送出失敗，請檢查網路後再試");return false}}
    ...
    function restoreSettingsSnapshot(showToast=true){const snapshot=savedStateRef.current;if(!snapshot)return null;setData(current=>applyEditableSettings(current,snapshot));setNewPasswordDraft("");settingsDirtyRef.current=false;setCid(current=>snapshot.children.some(item=>item.id===current)?current:(snapshot.children[0]?.id||current));if(showToast)say("已取消未儲存的變更");return snapshot}
    function performSettingsIntent(intent:SettingsIntent,availableState:State=data){if(intent.kind==="tab")setTab(intent.value);else if(intent.kind==="child")setCid(availableState.children.some(item=>item.id===intent.value)?intent.value:(availableState.children[0]?.id||cid));else{setRole("孩子");setTab("首頁")}}
    ...
    async function saveAllSettings(){if(savingSettingsRef.current||imageUploading)return false;if(newPasswordDraft&&newPasswordDraft.length<4){say("密碼至少 4 個字元");return false}if(!hasUnsavedChanges)return true;savingSettingsRef.current=true;setSavingSettings(true);const draft=clonePersistedState(data),next=prepareSettingsForSave(draft);try{return await persist(next,newPasswordDraft||undefined,{preserveDraftOnFailure:true,optimistic:false,successMessage:"✅ 已儲存",draftForRebase:draft})}finally{savingSettingsRef.current=false;setSavingSettings(false)}}
    ...
    async function runTaskAction(taskRecord:DailyTaskRecord,operation:"complete"|"approve"|"reject"|"skip"|"restore"|"undo"){
        ...
        }catch(error){await reloadState();say(error instanceof Error?error.message:"任務更新失敗")}finally{setTaskBusy("")}
    }
    function TaskChallenge(){
        const settings=taskSettingsForChild(data.dailyTaskSettings,cid),childRecords=data.dailyTaskRecords.filter(item=>item.childId===cid),todayRecords=childRecords.filter(item=>item.date===todayKey).sort((a,b)=>{const left=data.dailyTasks.find(task=>task.id===a.definitionId)?.sortOrder??999,right=data.dailyTasks.find(task=>task.id===b.definitionId)?.sortOrder??999;return left-right||a.titleSnapshot.localeCompare(b.titleSnapshot,"zh-TW")}),progress=taskProgress(todayRecords),todayGoalSettings=taskGoalSettingsForRecords(todayRecords,settings),weekly=weeklyTaskProgress(childRecords,todayKey),goal=goalResult(progress,todayGoalSettings),streak=calculateTaskStreak(childRecords,settings,todayKey),completedEntries=data.entries.filter(entry=>entry.childId===cid&&(entry.status??"completed")==="completed"&&entryDateKey(entry)===todayKey),fixedStars=completedEntries.filter(entry=>entry.type==="star"&&entry.sourceType==="daily_task").reduce((sum,entry)=>sum+entry.amount,0),otherStars=completedEntries.filter(entry=>entry.type==="star"&&entry.sourceType!=="daily_task").reduce((sum,entry)=>sum+entry.amount,0),deductStars=completedEntries.filter(entry=>entry.type==="deduct").reduce((sum,entry)=>sum+entry.amount,0),net=fixedStars+otherStars-deductStars,pending=todayRecords.filter(item=>item.status==="pending"||item.status==="pending_approval"),approvalBacklog=childRecords.filter(item=>item.status==="pending_approval"&&item.date<todayKey).sort((a,b)=>a.date.localeCompare(b.date)),finished=todayRecords.filter(item=>item.status==="completed"||item.status==="skipped"),hasDefinitions=data.dailyTasks.some(task=>task.childId===cid),remaining=Math.max(0,goal.required-progress.completed);
        const renderTaskCard=(item:DailyTaskRecord)=>{const busy=taskBusy===item.id,locked=Boolean(taskBusy),statusText=item.status==="completed"?"已完成":item.status==="skipped"?"今日不適用":item.status==="pending_approval"?"等待家長確認":"尚未完成";return <article key={item.id} className={`task-card task-${item.status}`} aria-busy={busy}><div className="task-card-copy"><span className="task-card-icon" aria-hidden="true">{item.status==="completed"?"✅":item.status==="skipped"?"⏭️":item.iconSnapshot}</span><div><h3>{item.titleSnapshot}</h3><p>{item.status==="completed"?`已獲得 ${item.rewardStarsSnapshot} ⭐`:`完成可得 ${item.rewardStarsSnapshot} ⭐`}</p><small>{item.date!==todayKey&&`${formatTaipeiDate(item.date)}・`}{statusText}{item.completedAt&&`・完成時間 ${formatTaipeiTime(item.completedAt)}`}{item.requestedAt&&item.status==="pending_approval"&&`・送出時間 ${formatTaipeiTime(item.requestedAt)}`}</small></div></div><div className="task-card-actions">{role==="孩子"&&item.status==="pending"&&<button className="task-primary" disabled={locked} onClick={()=>setTaskConfirm(item)}>{busy?"送出中…":"完成任務"}</button>}{role==="孩子"&&item.status==="pending_approval"&&<button disabled>⏳ 等待家長確認</button>}{role==="家長"&&item.status==="pending"&&<><button className="task-primary" disabled={locked} onClick={()=>runTaskAction(item,"complete")}>標記完成</button><button disabled={locked} onClick={()=>runTaskAction(item,"skip")}>今日不適用</button></>}{role==="家長"&&item.status==="pending_approval"&&<><button className="task-primary" disabled={locked} onClick={()=>runTaskAction(item,"approve")}>確認完成</button><button disabled={locked} onClick={()=>runTaskAction(item,"reject")}>退回</button><button disabled={locked} onClick={()=>runTaskAction(item,"skip")}>今日不適用</button></>}{role==="家長"&&item.status==="completed"&&<button className="task-undo" disabled={locked} onClick={()=>confirm(`確定撤銷「${item.titleSnapshot}」？星星也會同步扣回。`)&&runTaskAction(item,"undo")}>撤銷完成</button>}{role==="家長"&&item.status==="skipped"&&<button disabled={locked} onClick={()=>runTaskAction(item,"restore")}>恢復為待完成</button>}</div></article>};
        async function refreshTasks(){if(taskBusy)return;setTaskBusy("refresh");try{say(await reloadState()?"今日任務已刷新":"刷新失敗，請檢查網路")}finally{setTaskBusy("")}}
        return <div className="task-challenge"><section className="task-challenge-head"><div><p className="eyebrow">DAILY MISSION</p><h2>🎯 {child.name} 的今日任務</h2><p>{formatTaipeiDate(todayKey)}</p></div><button disabled={Boolean(taskBusy)} onClick={refreshTasks}>{taskBusy==="refresh"?"刷新中…":"↻ 刷新任務"}</button></section><div className="task-stat-grid"><article><span>✨ 今日獲得</span><strong className={net<0?"summary-deduct":""}>{net>0?"＋":""}{net} ⭐</strong><small>固定任務 ＋{fixedStars}・其他獎勵 ＋{otherStars}・扣星 −{deductStars}</small></article><article><span>🔥 連續達標</span><strong>{streak} 天</strong><small>{!progress.total?"今天沒有任務，不影響連續紀錄":goal.met?`今日已達標，連續第 ${streak} 天！`:`再完成 ${remaining} 項即可達標`}</small></article><article><span>📈 本週完成率</span><strong>{weekly.percentage===null?"—":`${weekly.percentage}%`}</strong><small>{weekly.total?`${weekly.completed} / ${weekly.total} 項（週一至今天）`:"本週尚無有效任務"}</small><div className="task-mini-progress"><i style={{width:`${weekly.percentage??0}%`}}/></div></article></div><section className="task-progress-panel"><div><h2>今日進度</h2>{progress.total?<p>{progress.completed} / {progress.total} 項完成</p>:<p>今天沒有安排任務</p>}</div>{progress.percentage!==null&&<strong>{progress.percentage}%</strong>}<div className="task-progress-track" role="progressbar" aria-label="今日任務完成率" aria-valuemin={0} aria-valuemax={100} aria-valuenow={progress.percentage??0}><i style={{width:`${progress.percentage??0}%`}}/></div></section>{approvalBacklog.length>0&&<section><Title text="待家長確認（含過往）"/><div className="task-card-grid">{approvalBacklog.map(renderTaskCard)}</div></section>}{!todayRecords.length?<section className="task-empty"><div>🎯</div><h2>{hasDefinitions?"今天沒有安排任務":"還沒有今日任務"}</h2><p>{hasDefinitions?"今日沒有符合星期排程的啟用任務，不影響連續達標。":"請家長到「家庭設定 → 每日任務設定」新增固定任務。"}</p>{role==="家長"&&<button className="primary" onClick={()=>goTab("家庭設定")}>{hasDefinitions?"調整每日任務設定":"前往新增每日任務"}</button>}</section>:<><section><Title text="今日待完成"/>{pending.length?<div className="task-card-grid">{pending.map(renderTaskCard)}</div>:<p className="empty">今天的待辦任務都處理完成了！</p>}</section><section><Title text="今日已完成／不適用"/>{finished.length?<div className="task-card-grid">{finished.map(renderTaskCard)}</div>:<p className="empty">完成任務後會顯示在這裡</p>}</section></>}</div>;
    }
    ...
    function removeEntry(e: Entry) { if(e.sourceType==="daily_task")return say("每日任務紀錄請到「任務挑戰」撤銷，星星會同步修正");const delta = e.type === "star" ? e.amount : e.type === "deduct" ? -e.amount : 0; persist({ ...data, entries: data.entries.filter(x => x.id !== e.id), children: data.children.map(c => c.id === e.childId ? { ...c, stars: Math.max(0, c.stars - delta) } : c),specialRewards:e.type==="special"&&e.status!=="pending"?specialStock(data.specialRewards,e.title,-e.amount):data.specialRewards }); }
    ...
    function Entries() { const rows = data.entries.filter(x => x.childId === cid); return <div className="entries">{rows.map(e => <article key={e.id} className={e.status==="revoked"?"entry-revoked":""}><div className="entry-icon">{e.sourceType==="daily_task"?"📋":e.type === "special" ? "🎉" : e.type === "star" ? "✨" : "📝"}</div><div className="entry-copy"><h3>{e.title}{e.type==="special"&&` × ${e.amount}`}</h3><small>{e.date} · {e.status==="pending"?"等待家長確認":e.status==="revoked"?"已撤銷":"已完成"}</small></div><strong className={e.type === "deduct" ? "red" : ""}>{e.status==="pending"?"待確認":e.status==="revoked"?"已撤銷":e.type === "special" ? "特殊獎勵" : `${e.type === "star" ? "+" : "−"}${e.amount} ★`}</strong>{role === "家長"&&e.status==="pending"&&<button className="primary" onClick={()=>approveEntry(e)}>確認</button>}{role === "家長"&&e.sourceType!=="daily_task"&&e.status!=="revoked"&&<button className="delete" onClick={() => confirm("確定刪除這筆紀錄？") && removeEntry(e)}>刪除</button>}</article>)}{!rows.length && <p className="empty">尚無紀錄</p>}</div>; }
    ...
    function Settings() {
        const c=child,tasks=data.dailyTasks.filter(task=>task.childId===cid).sort((a,b)=>a.sortOrder-b.sortOrder),taskSettings=taskSettingsForChild(data.dailyTaskSettings,cid);
        ...
        const addTask=()=>{const instant=new Date().toISOString(),task:DailyTaskDefinition={id:crypto.randomUUID(),childId:cid,title:"新每日任務",icon:"⭐",rewardStars:1,weekdays:[1,2,3,4,5,6,7],enabled:true,sortOrder:tasks.length,createdAt:instant,updatedAt:instant,scheduleStart:todayKey};editSettings(current=>({...current,dailyTasks:[...current.dailyTasks,task]}))};
