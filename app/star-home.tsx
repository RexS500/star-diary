...
export default function App({account}:{account:SignedInAccount}) {
    ...
    async function runTaskAction(taskRecord:DailyTaskRecord,operation:"complete"|"backfill"|"approve"|"reject"|"skip"|"restore"|"undo"){
        if(taskBusy)return;setTaskBusy(taskRecord.id);
        try{
            const childAction=role==="孩子"&&operation==="complete",backfillAction=role==="家長"&&operation==="backfill",payload=childAction?{action:"child_daily_task_complete",recordId:taskRecord.id,childId:cid}:backfillAction?{action:"parent_daily_task_backfill",recordId:taskRecord.id,childId:cid,password}:{action:"parent_daily_task_action",recordId:taskRecord.id,operation,password},response=await authenticatedFetch("/api/state",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(payload)}),result=await response.json();
            ...
            else if(backfillAction)say(`✅ 已補登昨天任務｜${taskRecord.titleSnapshot}｜＋${taskRecord.rewardStarsSnapshot} ⭐｜已記錄於 ${taskRecord.date.replaceAll("-","/")}`);
    ...
    function TaskChallenge(){
        ...
        return <div key={selectedDate} className={`task-challenge task-date-view${isYesterday?" is-yesterday":" is-today"}`}>
            ...
            {!selectedRecords.length?<section className="task-empty"><div>{isYesterday?"📝":"🎯"}</div><h2>{hasDefinitions?`${dayLabel}沒有安排每日任務`:"尚未建立每日任務"}</h2><p>{hasDefinitions?`${formatTaipeiDate(selectedDate)} 沒有符合排程的任務。`:"請家長到「家庭設定 → 每日任務設定」新增固定任務。"}</p>{!hasDefinitions&&role==="家長"&&<button className="primary" onClick={()=>goTab("家庭設定")}>前往新增每日任務</button>}</section>:<><section><Title text={`${dayLabel}${isYesterday?"待補登":"待完成"}`}/>{pending.length?<div className="task-card-grid">{pending.map(renderTaskCard)}</div>:<p className="empty">{isYesterday?"昨天的待補登任務都處理完成了！":"今天的待辦任務都處理完成了！"}</p>}</section><section><Title text={`${dayLabel}已完成／不適用`}/>{finished.length?<div className="task-card-grid">{finished.map(renderTaskCard)}</div>:<p className="empty">完成任務後會顯示在這裡</p>}</section></>}
        </div>;
    }
