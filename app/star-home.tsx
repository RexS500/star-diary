...
export default function App() {
    ...
    async function reloadState(){try{const response=await fetch(`/api/state?t=${Date.now()}`,{cache:"no-store"}),result=await response.json();if(!response.ok||!result.state)throw new Error(result.error||"無法讀取資料");const next=result.state as State;setData(next);setCid(current=>next.children.some(item=>item.id===current)?current:(next.children[0]?.id||current));setPasswordSet(Boolean(result.passwordSet));return true}catch{return false}}
