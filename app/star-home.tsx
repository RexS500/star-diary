...
const fallback: State = { children: [{ id: "c1", name: "小宇", gender: "boy", avatar: "boy", stars: 0 }], entries: [], rewards: [{ id: "r1", icon: "🍦", name: "冰淇淋", cost: 12, stock: 0 }], templates: [{ id: "t1", title: "主動整理書包", amount: 3, type: "star" }], redemptions: [],specialRewards:[],rewardIconLibrary:[] };
...
const entryDateKey = (entry:Entry) => entry.createdAt&&Number.isFinite(entryTime(entry.createdAt))?entryDay(entry.createdAt):entryDay(entry.date);
...
export default function App() {
    const [data, setData] = useState(fallback), [cid, setCid] = useState("c1"), [tab, setTab] = useState("首頁"), [role, setRole] = useState("孩子"), [password, setPassword] = useState(""), [passwordSet, setPasswordSet] = useState(false), [login, setLogin] = useState(false), [record, setRecord] = useState(false), [redeem, setRedeem] = useState<Reward | null>(null), [quickConfirm, setQuickConfirm] = useState<Template | null>(null), [toast, setToast] = useState(""), [loading, setLoading] = useState(true);
    ...
    async function reloadState(){try{const response=await fetch(`/api/state?t=${Date.now()}`,{cache:"no-store"}),result=await response.json();if(!response.ok||!result.state)throw new Error(result.error||"無法讀取資料");const loaded=result.state as State,next:State={...loaded,rewardIconLibrary:Array.isArray(loaded.rewardIconLibrary)?loaded.rewardIconLibrary:[]};setData(next);setCid(current=>next.children.some(item=>item.id===current)?current:(next.children[0]?.id||current));setPasswordSet(Boolean(result.passwordSet));return true}catch{return false}}
    ...
    async function uploadReward(file:File,id:string){const fingerprint=await rewardFileHash(file),reused=fingerprint?data.rewardIconLibrary.find(asset=>asset.hash===fingerprint):undefined,url=reused?.image||await uploadImage(file,"reward");if(!url)return;const reward=data.rewards.find(item=>item.id===id),identity=rewardImageIdentity(url),exists=data.rewardIconLibrary.find(asset=>rewardImageIdentity(asset.image)===identity),asset=exists||{id:crypto.randomUUID(),name:reward?.name.trim()?`${reward.name.trim()}圖片`:`自訂圖片 ${String(data.rewardIconLibrary.length+1).padStart(2,"0")}`,image:url,...(fingerprint?{hash:fingerprint}:{}),createdAt:new Date().toISOString()},library=exists?data.rewardIconLibrary:[...data.rewardIconLibrary,asset],ok=await persist({...data,rewardIconLibrary:library,rewards:data.rewards.map(item=>item.id===id?{...item,image:url}:item)});if(ok)say(reused?"已套用圖示庫中的相同圖片":"圖片已上傳並加入「我的圖片」")}
    ...
    function chooseBuiltinRewardIcon(id:string,icon:string){setData(current=>({...current,rewards:current.rewards.map(reward=>reward.id===id?{...reward,icon,image:undefined}:reward)}))}
    function chooseUploadedRewardIcon(id:string,image:string){setData(current=>({...current,rewards:current.rewards.map(reward=>reward.id===id?{...reward,image}:reward)}))}
    ...
    <section className="settings-card wide quick-indicator-settings"><h2>✨ 常用快速指標</h2><div className="template-card-list">{data.templates.map((t,index) => {const typeName=t.type==="star"?"加星指標":t.type==="deduct"?"扣星指標":"特殊獎勵",typeIcon=t.type==="star"?"⭐":t.type==="deduct"?"➖":"🎁";return <article className={`template-settings-card template-${t.type}`} key={t.id}><h3><span aria-hidden="true">{typeIcon}</span> {typeName} {String(index+1).padStart(2,"0")}</h3><div className="template-card-fields"><label className="template-field">類型<select value={t.type} onChange={e => setData({ ...data, templates: data.templates.map(x => x.id === t.id ? { ...x, type: e.target.value as Template["type"] } : x) })}><option value="star">加星</option><option value="deduct">扣星</option><option value="special">特殊獎勵</option></select></label><label className="template-field">指標／獎勵內容<input value={t.title} onChange={e => setData({ ...data, templates: data.templates.map(x => x.id === t.id ? { ...x, title: e.target.value } : x) })}/></label><label className="template-field">數量<input type="number" min="1" value={t.amount} onChange={e => setData({ ...data, templates: data.templates.map(x => x.id === t.id ? { ...x, amount: Math.max(1,Math.floor(+e.target.value||1)) } : x) })}/></label><div className="template-card-actions"><button type="button" className="template-delete" aria-label={`刪除${typeName} ${String(index+1).padStart(2,"0")}`} onClick={() => setData({ ...data, templates: data.templates.filter(x => x.id !== t.id) })}>刪除此項 🗑</button></div></div></article>})}</div><button className="add-line" onClick={() => setData({ ...data, templates: [...data.templates, { id: crypto.randomUUID(), title: "新指標", amount: 1, type: "star" }] })}>＋ 新增快速指標</button></section>
    <section className="settings-card wide reward-settings"><h2>🎁 星星寶庫</h2><p className="reward-settings-help">可選擇內建圖示，或上傳圖片並保存到「我的圖片」重複使用。</p><div className="reward-settings-list">{data.rewards.map((reward,index)=>{const number=String(index+1).padStart(2,"0"),titleId=`reward-setting-${reward.id}`;return <article className="reward-settings-card" key={reward.id} aria-labelledby={titleId}><h3 id={titleId}>🎁 獎品 {number}</h3><div className="reward-settings-fields"><div className="reward-media-field"><span className="reward-control-label">圖示／圖片</span>{renderRewardIconPicker(reward)}</div><label>獎品名稱<input value={reward.name} onChange={event=>setData({...data,rewards:data.rewards.map(item=>item.id===reward.id?{...item,name:event.target.value}:item)})}/></label><label>需要星星<input type="number" min="1" step="1" value={positiveInt(reward.cost)} onChange={event=>setData({...data,rewards:data.rewards.map(item=>item.id===reward.id?{...item,cost:positiveInt(event.target.value)}:item)})}/></label><div className="reward-settings-actions"><button type="button" className="reward-settings-delete" aria-label={`刪除獎品 ${number}`} onClick={()=>confirm(`確定刪除「${reward.name}」？`)&&setData({...data,rewards:data.rewards.filter(item=>item.id!==reward.id)})}>刪除此項 🗑</button></div></div></article>})}</div><button className="add-line" onClick={() => setData({ ...data, rewards: [...data.rewards, { id: crypto.randomUUID(), icon: "🎁", name: "新獎品", cost: 10, stock: 0 }] })}>＋ 新增獎品</button></section><button className="settings-save" onClick={() => persist(data)}>儲存所有設定</button></div>; }
}
function Avatar({ c }: {
    c: Child;
