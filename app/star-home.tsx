...
export default function App() {
    ...
    function addEntry(title: string, amount: number, type: Entry["type"]) { const e = { id: crypto.randomUUID(), childId: cid, title, amount: Math.abs(amount), type, date: now() }, delta=type==="special"?0:amount, next = { ...data, entries: [e, ...data.entries], children: data.children.map(c => c.id === cid ? { ...c, stars: Math.max(0, c.stars + delta) } : c) }; persist(next); }
    ...
    function Entries() { const rows = data.entries.filter(x => x.childId === cid); return <div className="entries">{rows.map(e => <article key={e.id}><div className="entry-icon">{e.type === "special" ? "🎉" : e.type === "star" ? "✨" : "📝"}</div><div className="entry-copy"><h3>{e.title}</h3><small>{e.date}</small></div><strong className={e.type === "deduct" ? "red" : ""}>{e.type === "special" ? "特殊獎勵" : `${e.type === "star" ? "+" : "−"}${e.amount} ★`}</strong>{role === "家長" && <button className="delete" onClick={() => confirm("確定刪除這筆紀錄？") && removeEntry(e)}>刪除</button>}</article>)}{!rows.length && <p className="empty">尚無紀錄</p>}</div>; }
    ...
    <section className="settings-card wide"><h2>🎁 獎品池</h2><div className="table-head"><span>圖示</span><span>獎品名稱</span><span>星星</span><span>現有數量</span></div><div className="edit-list">{data.rewards.map(r => <div key={r.id}><input value={r.icon} onChange={e=>setData({...data,rewards:data.rewards.map(x=>x.id===r.id?{...x,icon:e.target.value}:x)})}/><input value={r.name} onChange={e => setData({ ...data, rewards: data.rewards.map(x => x.id === r.id ? { ...x, name: e.target.value } : x) })}/><input type="number" value={r.cost} onChange={e => setData({ ...data, rewards: data.rewards.map(x => x.id === r.id ? { ...x, cost: +e.target.value } : x) })}/><input type="number" value={r.stock} onChange={e => setData({ ...data, rewards: data.rewards.map(x => x.id === r.id ? { ...x, stock: +e.target.value } : x) })}/></div>)}</div><button className="add-line" onClick={() => setData({ ...data, rewards: [...data.rewards, { id: crypto.randomUUID(), icon: "🎁", name: "新獎品", cost: 10, stock: 0 }] })}>＋ 新增獎品</button></section><button className="settings-save" onClick={() => persist(data)}>儲存所有設定</button></div>; }
}
function Avatar({ c }: {
    c: Child;
