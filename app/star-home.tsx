...
function RecordModal({templates,allowBackdate,onClose,onSave,onValidationError}:{templates:Template[];allowBackdate:boolean;onClose:()=>void;onSave:(title:string,amount:number,type:Entry["type"],occurredAt?:string)=>Promise<boolean>;onValidationError:(message:string)=>void}){
    const [t,setT]=useState<Entry["type"]>("star"),[name,setName]=useState(""),[n,setN]=useState(0),[saving,setSaving]=useState(false),[recordDate,setRecordDate]=useState(()=>taipeiDateTimeInput().date),[recordTime,setRecordTime]=useState(()=>taipeiDateTimeInput().time);
    ...
    return <div ref={backdropRef} className="modal-back record-modal-back">
        <section className="modal record-modal" role="dialog" aria-modal="true" aria-labelledby="record-modal-title" aria-busy={saving} onFocusCapture={focusField}>
            ...
            <div className="toggle three">{RECORD_TYPES.map(item=><button type="button" key={item.value} className={t===item.value?"chosen":""} disabled={saving} onClick={()=>{setT(item.value);setN(item.value==="special"?1:0)}}>{item.label}</button>)}</div>
            {picks.length>0&&<div className="quick-picks"><b>快速選取</b>{picks.map(item=>{const count=Math.max(1,Math.abs(Math.floor(Number(item.amount)||1)));return <button type="button" key={item.id} className={item.type==="deduct"?"deduct-pick":""} disabled={saving} onClick={()=>{setName(item.title);setN(count)}}>{item.title} × {count}</button>})}</div>}
...
export default function App() {
    ...
    function restoreSettingsSnapshot(showToast=true){const snapshot=savedStateRef.current;if(!snapshot)return null;setData(current=>applyEditableSettings(current,snapshot));settingsDirtyRef.current=false;setCid(current=>snapshot.children.some(item=>item.id===current)?current:(snapshot.children[0]?.id||current));if(showToast)say("已取消未儲存的變更");return snapshot}
    ...
    async function addEntry(title: string, amount: number, type: Entry["type"], selectedOccurredAt?:string) {
        const count=positiveInt(amount),signed=type==="deduct"?-count:count,pending=role==="孩子",createdAt=new Date().toISOString(),occurredAt=role==="家長"?(selectedOccurredAt||createdAt):undefined;
        if(occurredAt&&Date.parse(occurredAt)>Date.now()){say("紀錄時間不可晚於現在");return false}
        const e:Entry = { id: crypto.randomUUID(), childId: cid, title:title.trim(), amount: count, type, date: formatTaipeiOccurrence(occurredAt||createdAt),...(occurredAt?{occurredAt}:{}),createdAt,status:pending?"pending":"completed" };
        const delta=pending||type==="special"?0:signed,next = { ...data, entries: [e, ...data.entries], children: data.children.map(c => c.id === cid ? { ...c, stars: Math.max(0, c.stars + delta) } : c),specialRewards:!pending&&type==="special"?specialStock(data.specialRewards,title,count):data.specialRewards };
