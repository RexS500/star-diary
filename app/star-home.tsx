...
export default function App() {
    ...
    function specialStock(list:Reward[],title:string,amount:number){const found=list.find(r=>r.name.trim()===title.trim());return found?list.map(r=>r.id===found.id?{...r,stock:Math.max(0,r.stock+amount)}:r):amount>0?[...list,{id:crypto.randomUUID(),icon:"🎁",name:title.trim(),cost:0,stock:amount}]:list}
    function addEntry(title: string, amount: number, type: Entry["type"]) { const pending=role==="孩子",e:Entry = { id: crypto.randomUUID(), childId: cid, title, amount: Math.abs(amount), type, date: now(),status:pending?"pending":"completed" }, delta=pending||type==="special"?0:amount, next = { ...data, entries: [e, ...data.entries], children: data.children.map(c => c.id === cid ? { ...c, stars: Math.max(0, c.stars + delta) } : c),rewards:!pending&&type==="special"?specialStock(data.rewards,title,Math.abs(amount)):data.rewards }; if(pending)submitPending("child_entry",e,next);else persist(next); }
    ...
    function removeEntry(e: Entry) { const delta = e.type === "star" ? e.amount : e.type === "deduct" ? -e.amount : 0; persist({ ...data, entries: data.entries.filter(x => x.id !== e.id), children: data.children.map(c => c.id === e.childId ? { ...c, stars: Math.max(0, c.stars - delta) } : c),rewards:e.type==="special"&&e.status!=="pending"?specialStock(data.rewards,e.title,-e.amount):data.rewards }); }
    function approveEntry(e:Entry){const delta=e.type==="star"?e.amount:e.type==="deduct"?-e.amount:0;persist({...data,entries:data.entries.map(x=>x.id===e.id?{...x,status:"completed"}:x),children:data.children.map(x=>x.id===e.childId?{...x,stars:Math.max(0,x.stars+delta)}:x),rewards:e.type==="special"?specialStock(data.rewards,e.title,e.amount):data.rewards})}
