...
function RecordModal({templates,onClose,onSave,onValidationError}:{templates:Template[];onClose:()=>void;onSave:(title:string,amount:number,type:Entry["type"])=>Promise<boolean>;onValidationError:(message:string)=>void}){
    const [t,setT]=useState<Entry["type"]>("star"),[name,setName]=useState(""),[n,setN]=useState(0),[saving,setSaving]=useState(false);
    ...
    async function save(){
        ...
        if(!Number.isFinite(n)||n<1)return onValidationError(t==="special"?"請填寫獎勵數量":"請選擇星星數量");
        setSaving(true);
        try{if(await onSave(name,t==="deduct"?-n:n,t))onClose()}finally{setSaving(false)}
    }
...
export default function App() {
    ...
    const [crop,setCrop]=useState<CropTarget|null>(null),[todayKey,setTodayKey]=useState(taipeiDateKey()),[taskBusy,setTaskBusy]=useState(""),[taskConfirm,setTaskConfirm]=useState<DailyTaskRecord|null>(null),[taskSyncError,setTaskSyncError]=useState(false);
    ...
    function quick(t: Template) { const count=Math.max(1,Math.abs(Math.floor(Number(t.amount)||1))),amount = t.type === "deduct" ? -count : count; void addEntry(t.title, amount, t.type); }
