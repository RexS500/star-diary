...
const taskHealthLabels:Record<TaskHealthMetric["healthStatus"],string>={insufficient_data:"資料不足",established:"已建立",stable:"穩定",developing:"發展中",observe:"持續觀察",needs_review:"需要檢視"};
const maturityLabels:Record<TaskHealthMetric["maturityStatus"],string>={building:"建立中",developing:"發展中",stable:"穩定",established:"已養成",graduated:"已畢業"};
const taskWeekdayLabels=["","一","二","三","四","五","六","日"];

function TaskCompletionWeekCard({period,days,scaleMaximum,onSelect}:{period:WeekPeriod;days:DailyTaskCompletionMetric[];scaleMaximum:number;onSelect:(day:DailyTaskCompletionMetric)=>void}){
    const width=700,height=270,baseY=205,plotHeight=155,step=100;
    const points=days.flatMap((day,index)=>day.completionRate===null||day.isFuture?[]:[`${50+index*step},${baseY-day.completionRate/100*plotHeight}`]).join(" ");
    const rate=calculateWeightedCompletionRate(days,{excludeTodayInProgress:true});
    const completed=days.reduce((sum,day)=>sum+day.completedCount,0),scheduled=days.reduce((sum,day)=>sum+day.completedCount+day.missedCount,0);
    return <article className="task-completion-week-card">
        <div className="analytics-card-heading"><div><h3>{period.label}</h3><p>{formatWeekRange(period)}</p></div><div className="task-week-rate"><strong>{rate===null?"—":`${rate}%`}</strong><small>{completed}/{scheduled} 項</small></div></div>
        <div className="task-completion-legend"><span><i className="is-scheduled"/>排定</span><span><i className="is-completed"/>完成</span><span><i className="is-rate"/>達成率</span></div>
        <svg className="task-completion-chart" viewBox={`0 0 ${width} ${height}`} role="img" aria-label={`${period.label}每日任務排定、完成與達成率`}>
            {[0,25,50,75,100].map(value=><g key={value}><line x1="22" x2="678" y1={baseY-value/100*plotHeight} y2={baseY-value/100*plotHeight}/><text x="4" y={baseY-value/100*plotHeight+4}>{value}</text></g>)}
            {days.map((day,index)=>{const x=50+index*step,scheduledHeight=day.scheduledCount/scaleMaximum*plotHeight,completedHeight=day.completedCount/scaleMaximum*plotHeight;return <g key={day.date} className={day.isFuture?"is-future":undefined}><rect className="scheduled-bar" x={x-20} y={baseY-scheduledHeight} width="40" height={scheduledHeight}/><rect className="completed-bar" x={x-13} y={baseY-completedHeight} width="26" height={completedHeight}/>{day.completionRate!==null&&!day.isFuture&&<circle className="rate-point" cx={x} cy={baseY-day.completionRate/100*plotHeight} r="6"/>}<text className="task-chart-count" x={x} y={Math.max(20,baseY-scheduledHeight-8)} textAnchor="middle">{day.completedCount}/{day.scheduledCount}</text></g>})}
            {points&&<polyline className="task-rate-line" points={points}/>}
        </svg>
        <div className="task-completion-day-buttons">{days.map(day=><button type="button" key={day.date} className={day.isFuture?"is-future":undefined} onClick={()=>onSelect(day)}><strong>{taskWeekdayLabels[day.weekday]}</strong><small>{shortAnalyticsDate(day.date)}</small>{day.isFuture?<em>未來</em>:day.isTodayInProgress?<em>進行中</em>:<span>{day.completionRate===null?"—":`${day.completionRate}%`}</span>}</button>)}</div>
    </article>;
}

function TaskHealthDetail({metric,isParent,onAdjust,onGraduate,onClose}:{metric:TaskHealthMetric;isParent:boolean;onAdjust:(id:string)=>void;onGraduate:(metric:TaskHealthMetric)=>void;onClose:()=>void}){
    const recent=[...metric.executions].filter(item=>item.status!=="not_applicable").sort((a,b)=>b.date.localeCompare(a.date)).slice(0,14);
    return <div className="modal-back"><section className="modal task-health-modal" role="dialog" aria-modal="true" aria-labelledby="task-health-detail-title"><button className="close" aria-label="關閉任務健康度明細" onClick={onClose}>×</button><h2 id="task-health-detail-title">{metric.icon} {metric.title}</h2><div className="task-health-detail-grid"><span><small>完成率</small><strong>{metric.completionRate===null?"—":`${metric.completionRate}%`}</strong></span><span><small>排定／完成</small><strong>{metric.scheduledCount}／{metric.completedCount}</strong></span><span><small>目前連續完成</small><strong>{metric.currentCompletionStreak} 次</strong></span><span><small>目前連續未完成</small><strong>{metric.currentMissStreak} 次</strong></span><span><small>最長連續完成</small><strong>{metric.longestCompletionStreak} 次</strong></span><span><small>補登比例</small><strong>{Math.round(metric.backfillRatio*100)}%</strong></span></div><div className="task-health-recommendations"><h3>任務調整建議</h3>{metric.recommendations.map(item=><p key={item}>{item}</p>)}</div>{recent.length>0&&<div className="task-health-history"><h3>最近執行情形</h3>{recent.map(item=><p key={`${item.definitionId}-${item.date}`}><span>{item.date.replaceAll("-","/")}</span><strong className={`is-${item.status}`}>{item.status==="completed"?item.backfilled?"已完成・補登":"已完成":item.status==="in_progress"?"進行中":"未完成"}</strong></p>)}</div>}{isParent&&metric.currentTask&&!metric.graduated&&<div className="task-health-modal-actions"><button type="button" className="secondary" onClick={()=>onAdjust(metric.definitionId)}>前往調整任務</button>{metric.maturityStatus==="established"&&<button type="button" className="primary" onClick={()=>onGraduate(metric)}>標記為已養成習慣</button>}</div>}{!metric.currentTask&&<p className="task-history-note">這是已刪除任務的歷史紀錄，僅保留分析，無法再調整設定。</p>}</section></div>;
}

function Analytics({data,child,onRefresh,todayKey,isParent,onAdjustTask,onHabitAction}:{data:State;child:Child;onRefresh:()=>Promise<State|null>;todayKey:string;isParent:boolean;onAdjustTask:(taskId:string)=>void;onHabitAction:(taskId:string,operation:"graduate"|"reactivate")=>Promise<boolean>}){
    ...
    const [healthDays,setHealthDays]=useState<14|30|90>(30),[selectedTaskDay,setSelectedTaskDay]=useState<DailyTaskCompletionMetric|null>(null),[selectedHealth,setSelectedHealth]=useState<TaskHealthMetric|null>(null),[habitConfirm,setHabitConfirm]=useState<TaskHealthMetric|null>(null),[habitBusy,setHabitBusy]=useState(false),[dismissedRecommendations,setDismissedRecommendations]=useState<Set<string>>(()=>new Set());
    ...
    const taskWeeks=useMemo(()=>chartPeriods.map(period=>({period,days:report.dailyTaskCompletion.filter(day=>day.date>=period.start&&day.date<=period.end)})),[chartPeriods,report.dailyTaskCompletion]);
    const taskScaleMaximum=Math.max(1,...taskWeeks.flatMap(week=>week.days.map(day=>day.scheduledCount)));
    const taskRangeRate=calculateWeightedCompletionRate(report.dailyTaskCompletion,{excludeTodayInProgress:true}),taskFullStreak=calculateFullCompletionStreak(report.dailyTaskCompletion);
    const taskBestDay=[...report.dailyTaskCompletion].filter(day=>!day.isFuture&&!day.isTodayInProgress&&day.completionRate!==null).sort((a,b)=>(b.completionRate??-1)-(a.completionRate??-1)||b.completedCount-a.completedCount)[0];
    const taskAttentionDay=[...report.dailyTaskCompletion].filter(day=>!day.isFuture&&!day.isTodayInProgress&&day.completionRate!==null).sort((a,b)=>(a.completionRate??101)-(b.completionRate??101)||b.scheduledCount-a.scheduledCount)[0];
    const healthStart=addCalendarDays(todayKey,-(healthDays-1));
    const healthMetrics=useMemo(()=>buildTaskHealthMetrics({childId:child.id,start:healthStart,end:todayKey,todayKey,definitions:data.dailyTasks,records:data.dailyTaskRecords}),[child.id,healthStart,todayKey,data.dailyTasks,data.dailyTaskRecords]);
    const graduatedHabits=useMemo(()=>buildGraduatedHabitMetrics({childId:child.id,todayKey,definitions:data.dailyTasks,records:data.dailyTaskRecords}),[child.id,todayKey,data.dailyTasks,data.dailyTaskRecords]);
    ...
    useEffect(()=>{setSelectedTaskDay(null);setSelectedHealth(null);setHabitConfirm(null);setDismissedRecommendations(new Set())},[child.id]);
    async function confirmHabitGraduation(){if(!habitConfirm||habitBusy)return;setHabitBusy(true);try{if(await onHabitAction(habitConfirm.definitionId,"graduate")){setHabitConfirm(null);setSelectedHealth(null)}}finally{setHabitBusy(false)}}
    ...
    return <div className="analytics weekly-analytics">
        ...
        <section className="analytics-panel task-completion-analysis" aria-labelledby="task-completion-title"><div className="analytics-section-title"><div><h2 id="task-completion-title">📋 每日任務達成率分析</h2><p>排定與完成依歷史任務快照計算；補登歸到任務日期，今日未完成只顯示為進行中。</p></div></div><div className="task-completion-summary"><article><small>範圍加權達成率</small><strong>{taskRangeRate===null?"—":`${taskRangeRate}%`}</strong></article><article><small>連續全數完成</small><strong>{taskFullStreak} 天</strong></article><article><small>表現最好</small><strong>{taskBestDay?`${shortAnalyticsDate(taskBestDay.date)}・${taskBestDay.completionRate}%`:"—"}</strong></article><article><small>可多關注</small><strong>{taskAttentionDay?`${shortAnalyticsDate(taskAttentionDay.date)}・${taskAttentionDay.completionRate}%`:"—"}</strong></article></div><div className="task-completion-week-list">{taskWeeks.map(week=><TaskCompletionWeekCard key={week.period.key} period={week.period} days={week.days} scaleMaximum={taskScaleMaximum} onSelect={setSelectedTaskDay}/>)}</div>{selectedTaskDay&&<article className="task-day-detail" aria-live="polite"><header><div><h3>{formatTaipeiDate(selectedTaskDay.date)}</h3><p>完成 {selectedTaskDay.completedCount}／排定 {selectedTaskDay.scheduledCount}{selectedTaskDay.isTodayInProgress?"・今天仍在進行中":""}</p></div><button type="button" aria-label="關閉當日任務明細" onClick={()=>setSelectedTaskDay(null)}>×</button></header><div className="task-day-detail-groups"><div><h4>已完成</h4>{selectedTaskDay.executions.filter(item=>item.status==="completed").map(item=><p key={`${item.definitionId}-completed`}><span>{item.icon} {item.title}</span><small>{item.backfilled?item.backfillSource==="current_definition"?"依目前設定補登":"補登":"當日完成"}</small></p>)}{!selectedTaskDay.executions.some(item=>item.status==="completed")&&<p className="empty">沒有已完成任務</p>}</div><div><h4>{selectedTaskDay.isTodayInProgress?"進行中":"未完成"}</h4>{selectedTaskDay.executions.filter(item=>item.status==="missed"||item.status==="in_progress").map(item=><p key={`${item.definitionId}-pending`}><span>{item.icon} {item.title}</span><small>{item.status==="in_progress"?"尚未結算":"未完成"}</small></p>)}{!selectedTaskDay.executions.some(item=>item.status==="missed"||item.status==="in_progress")&&<p className="empty">沒有未完成任務</p>}</div><div><h4>不適用</h4>{selectedTaskDay.executions.filter(item=>item.status==="not_applicable").map(item=><p key={`${item.definitionId}-skipped`}><span>{item.icon} {item.title}</span><small>不列入達成率</small></p>)}{!selectedTaskDay.executions.some(item=>item.status==="not_applicable")&&<p className="empty">沒有不適用任務</p>}</div></div></article>}</section>
        <section className="analytics-panel task-health-analysis" aria-labelledby="task-health-title"><div className="analytics-section-title task-health-heading"><div><h2 id="task-health-title">🩺 任務健康度與調整建議</h2><p>依排定執行次數加權判斷；未排定日與不適用日不會被算成失敗。</p></div><label>分析期間<select value={healthDays} onChange={event=>setHealthDays(Number(event.target.value) as 14|30|90)}><option value={14}>最近 14 天</option><option value={30}>最近 30 天</option><option value={90}>最近 90 天</option></select></label></div>{healthMetrics.filter(metric=>!metric.graduated).length?<div className="task-health-grid">{healthMetrics.filter(metric=>!metric.graduated).map(metric=><article className={`task-health-card status-${metric.healthStatus}`} key={metric.definitionId}><header><div><span aria-hidden="true">{metric.icon}</span><div><h3>{metric.title}</h3><small>{metric.currentTask?"目前任務":"歷史任務"}</small></div></div><span className="task-health-badge">{taskHealthLabels[metric.healthStatus]}</span></header><div className="task-health-rate"><strong>{metric.completionRate===null?"—":`${metric.completionRate}%`}</strong><span>完成 {metric.completedCount}／排定 {metric.scheduledCount}</span></div><div className="task-health-streaks"><span>連續完成 <b>{metric.currentCompletionStreak}</b></span><span>連續未完成 <b>{metric.currentMissStreak}</b></span><span>成熟度 <b>{maturityLabels[metric.maturityStatus]}</b></span></div>{!dismissedRecommendations.has(metric.definitionId)&&<div className="task-health-suggestion"><p>{metric.recommendations[0]}</p><button type="button" aria-label={`略過${metric.title}的這則建議`} onClick={()=>setDismissedRecommendations(current=>new Set(current).add(metric.definitionId))}>略過</button></div>}<div className="task-health-card-actions"><button type="button" onClick={()=>setSelectedHealth(metric)}>查看分析</button>{isParent&&metric.currentTask&&<button type="button" onClick={()=>onAdjustTask(metric.definitionId)}>調整任務</button>}{isParent&&metric.currentTask&&metric.maturityStatus==="established"&&<button type="button" className="graduate-habit-button" onClick={()=>setHabitConfirm(metric)}>習慣畢業</button>}</div></article>)}</div>:<p className="empty">目前沒有可分析的任務。建立並執行每日任務後，這裡會逐步累積健康度。</p>}<div className="graduated-habits"><div><h3>🎓 已養成習慣</h3><p>畢業後不再每天排定或發放星星，歷史完成紀錄仍完整保留。</p></div>{graduatedHabits.length?<div className="graduated-habit-list">{graduatedHabits.map(habit=><article key={habit.definitionId}><span>{habit.icon}</span><div><strong>{habit.title}</strong><small>{habit.graduatedAt?`${habit.graduatedAt.slice(0,10).replaceAll("-","/")} 畢業`:"已畢業"}・畢業前 30 天 {habit.lastThirtyDaysRate===null?"資料不足":`${habit.lastThirtyDaysRate}%`}</small></div>{isParent&&<button type="button" disabled={habitBusy} onClick={()=>void onHabitAction(habit.definitionId,"reactivate")}>重新啟用</button>}</article>)}</div>:<p className="empty">目前還沒有畢業的習慣。達到穩定門檻後，可由家長手動確認。</p>}</div></section>
        ...
        {selectedHealth&&<TaskHealthDetail metric={selectedHealth} isParent={isParent} onAdjust={taskId=>{setSelectedHealth(null);onAdjustTask(taskId)}} onGraduate={metric=>{setSelectedHealth(null);setHabitConfirm(metric)}} onClose={()=>setSelectedHealth(null)}/>}
        {habitConfirm&&<div className="modal-back"><section className="modal habit-graduation-modal" role="dialog" aria-modal="true" aria-labelledby="habit-graduation-title"><button className="close" aria-label="關閉習慣畢業確認" disabled={habitBusy} onClick={()=>setHabitConfirm(null)}>×</button><h2 id="habit-graduation-title">🎓 確認習慣畢業</h2><p>「{habitConfirm.title}」畢業後將停止每天排定與發放星星，但所有歷史紀錄與分析都會保留。</p><div className="confirm-box"><p>最近完成率<strong>{habitConfirm.completionRate}%</strong></p><p>目前連續完成<strong>{habitConfirm.currentCompletionStreak} 次</strong></p><p>之後仍可<strong>從已養成習慣重新啟用</strong></p></div><div className="record-actions"><button type="button" className="save" disabled={habitBusy} onClick={()=>void confirmHabitGraduation()}>{habitBusy?"處理中…":"確認標記為已養成"}</button><button type="button" className="cancel-action" disabled={habitBusy} onClick={()=>setHabitConfirm(null)}>取消</button></div></section></div>}
    </div>;
}
export default function App({account}:{account:SignedInAccount}) {
    const [data, setData] = useState(fallback), [cid, setCid] = useState(""), [tab, setTab] = useState("首頁"), [role, setRole] = useState("孩子"), [password, setPassword] = useState(""), [passwordSet, setPasswordSet] = useState(false), [securityInfo,setSecurityInfo]=useState<SecurityInfo>({configured:false,questionType:"",questionText:"",hint:""}), [login, setLogin] = useState(false), [forgotPassword,setForgotPassword]=useState(false), [record, setRecord] = useState(false), [redeem, setRedeem] = useState<Reward | null>(null), [quickConfirm, setQuickConfirm] = useState<Template | null>(null), [toast, setToast] = useState(""), [loading, setLoading] = useState(true),[loadFailed,setLoadFailed]=useState(false),[refreshingData,setRefreshingData]=useState(false);
    const [crop,setCrop]=useState<CropTarget|null>(null),[todayKey,setTodayKey]=useState(taipeiDateKey()),[taskDateView,setTaskDateView]=useState<"today"|"yesterday">("today"),[taskBusy,setTaskBusy]=useState(""),[taskConfirm,setTaskConfirm]=useState<DailyTaskRecord|null>(null),[taskSyncError,setTaskSyncError]=useState(false),[officialLibraryOpen,setOfficialLibraryOpen]=useState(false);
    const [currentDefinitionConfirm,setCurrentDefinitionConfirm]=useState<DailyTaskRecord|null>(null);
    ...
    const revisionRef=useRef(0),settingsDirtyRef=useRef(false),settingsBusyRef=useRef(false),savingSettingsRef=useRef(false),savedStateRef=useRef<State|null>(null),intentTriggerRef=useRef<HTMLElement|null>(null),toastTimerRef=useRef<ReturnType<typeof setTimeout>|null>(null),refreshPromiseRef=useRef<Promise<AppRefreshResult>|null>(null);
    ...
    useEffect(()=>{settingsDirtyRef.current=hasUnsavedChanges;settingsBusyRef.current=hasUnsavedChanges||invalidIntegerFields.size>0||savingSettings||imageUploading},[hasUnsavedChanges,imageUploading,invalidIntegerFields.size,savingSettings]);
    ...
    useEffect(()=>{setTaskConfirm(null)},[todayKey]);
    useEffect(()=>{
    ...
    useEffect(()=>{
        ...
        if(parameters.get("auth_intent")==="create_family")say("此帳號已經是家庭成員，已開啟原本家庭");
        if(parameters.get("setup")==="dailyTasks"&&account.role!=="child"){setRole("家長");setTab("家庭設定");setSettingsTab("dailyTasks")}
        if(parameters.has("auth_intent")||parameters.has("setup")){
            parameters.delete("auth_intent");parameters.delete("setup");
    ...
    async function refreshAllData():Promise<AppRefreshResult>{
        if(refreshPromiseRef.current)return refreshPromiseRef.current;
        if(document.querySelector(".modal-back,[role='dialog']")){say("請先關閉目前的操作視窗，再重新整理資料");return{status:"cancelled"}}
        if(savingSettings||imageUploading){say(imageUploading?"圖片上傳中，請稍候再重新整理":"設定正在儲存，請稍候再重新整理");return{status:"cancelled"}}
        if(settingsBusyRef.current&&!window.confirm("目前有尚未儲存的變更，確定要重新整理嗎？"))return{status:"cancelled"};
        const operation=(async()=>{
            setRefreshingData(true);
            try{
                const sessionRequest=fetch(`/api/auth/session?t=${Date.now()}`,{cache:"no-store"}).then(response=>{if(!response.ok)throw new Error("登入資料更新失敗");return response.json()});
                const latest=await reloadState();
                if(!latest)throw new Error("家庭資料更新失敗");
                await sessionRequest;
                const detail:AppDataRefreshDetail={tasks:[]};
                window.dispatchEvent(new CustomEvent<AppDataRefreshDetail>(APP_DATA_REFRESH_EVENT,{detail}));
                const taskResults=await Promise.all(detail.tasks);
                if(taskResults.some(result=>!result))throw new Error("部分資料更新失敗");
                say(settingsDirtyRef.current?"資料已更新，尚未儲存的設定仍保留":"資料已更新");
                return{status:"success",message:"已更新"} as AppRefreshResult;
            }catch{
                say("更新資料失敗，請檢查網路後再試");
                return{status:"error",message:"更新失敗"} as AppRefreshResult;
            }finally{
    ...
    function performSettingsIntent(intent:SettingsIntent,availableState:State=data){
        if(intent.kind==="tab"){
            setTab(intent.value);
            if(intent.value==="任務挑戰"){setTaskDateView("today");void syncDailyTasks()}
    ...
    async function runTaskAction(taskRecord:DailyTaskRecord,operation:"complete"|"backfill"|"backfill_current_definition"|"approve"|"reject"|"skip"|"restore"|"undo"){
        ...
        try{
            ...
            else if(backfillAction)say(`✅ 已${operation==="backfill_current_definition"?"依目前設定":""}補登昨天任務｜${taskRecord.titleSnapshot}｜＋${taskRecord.rewardStarsSnapshot} ⭐｜已記錄於 ${taskRecord.date.replaceAll("-","/")}`);
            else say(operation==="skip"?`已標記${taskRecord.date===addCalendarDays(todayKey,-1)?"昨天":"今日"}不適用`:operation==="restore"?"已恢復為待完成":operation==="undo"?"已撤銷完成並修正星星":operation==="reject"?"已退回給孩子":"任務已完成");
        }catch(error){setTaskSyncError(!await reloadState());say(error instanceof Error?error.message:"任務更新失敗")}finally{setTaskBusy("")}
    }
    function adjustTaskFromAnalytics(taskId:string){
        setSettingsTab("dailyTasks");
        window.history.replaceState(window.history.state,"",`${window.location.pathname}${window.location.search}#dailyTasks`);
        goTab("家庭設定");
        window.setTimeout(()=>document.getElementById(`daily-task-setting-${taskId}`)?.scrollIntoView({block:"center",behavior:"smooth"}),80);
    }
    async function runHabitAction(taskId:string,operation:"graduate"|"reactivate"){
        if(taskBusy)return false;
        setTaskBusy(`habit-${taskId}`);
        try{
            const response=await authenticatedFetch("/api/state",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:"parent_daily_task_habit_action",definitionId:taskId,operation,password})}),result=await response.json();
            if(!response.ok)throw new Error(result.error||"習慣狀態更新失敗");
            applyServerPayload(result,{replaceSettings:true,establishBaseline:true});
            say(operation==="graduate"?"🎓 已標記為養成習慣，之後不再每日排定":"已重新啟用，將從今天開始排定任務");
            return true;
        }catch(error){say(error instanceof Error?error.message:"習慣狀態更新失敗");return false}finally{setTaskBusy("")}
    }
    ...
    function TaskChallenge(){
        const isYesterday=taskDateView==="yesterday",selectedDate=isYesterday?addCalendarDays(todayKey,-1):todayKey,settings=taskSettingsForChild(data.dailyTaskSettings,cid),childRecords=data.dailyTaskRecords.filter(item=>item.childId===cid),day=dailyTaskDayView(data.dailyTaskRecords,data.dailyTasks,cid,selectedDate,data.dailyTaskSortMode),selectedRecords=day.records,progress=day.progress,pending=day.pending,finished=day.finished,currentBackfillCandidates=isYesterday?currentDefinitionBackfillCandidates(data.dailyTasks,data.dailyTaskRecords,cid,selectedDate,data.dailyTaskSortMode):[],selectedGoalSettings=taskGoalSettingsForRecords(selectedRecords,settings),weekly=weeklyTaskProgress(childRecords,todayKey),goal=goalResult(progress,selectedGoalSettings),streak=calculateTaskStreak(childRecords,settings,todayKey),completedEntries=data.entries.filter(entry=>entry.childId===cid&&(entry.status??"completed")==="completed"&&entryDateKey(entry)===selectedDate),fixedStars=completedEntries.filter(entry=>entry.type==="star"&&entry.sourceType==="daily_task").reduce((sum,entry)=>sum+entry.amount,0),otherStars=completedEntries.filter(entry=>entry.type==="star"&&entry.sourceType!=="daily_task").reduce((sum,entry)=>sum+entry.amount,0),deductStars=completedEntries.filter(entry=>entry.type==="deduct").reduce((sum,entry)=>sum+entry.amount,0),net=fixedStars+otherStars-deductStars,approvalBacklog=childRecords.filter(item=>item.status==="pending_approval"&&item.date<todayKey&&item.date!==selectedDate).sort((a,b)=>a.date.localeCompare(b.date)),hasDefinitions=data.dailyTasks.some(task=>task.applicableChildIds.includes(cid)),remaining=Math.max(0,goal.required-progress.completed),dayLabel=isYesterday?"昨天":"今天";
        const renderTaskCard=(item:DailyTaskRecord)=>{
            const busy=taskBusy===item.id,locked=Boolean(taskBusy),backfilled=Boolean(item.backfilledAt),fromCurrentDefinition=item.backfillSource==="current_definition",currentDefinitionCandidate=fromCurrentDefinition&&item.isVirtualBackfillCandidate===true,statusText=item.status==="completed"?"已完成":item.status==="skipped"?`${isYesterday?"昨天":"今日"}不適用`:item.status==="pending_approval"?"等待家長確認":"尚未完成";
            return <article key={item.id} className={`task-card task-${item.status}${backfilled?" task-backfilled":""}${fromCurrentDefinition?" task-current-definition":""}`} aria-busy={busy}><div className="task-card-copy"><span className="task-card-icon" aria-hidden="true">{item.status==="completed"?"✅":item.status==="skipped"?"⏭️":item.iconSnapshot}</span><div><h3>{item.titleSnapshot}{fromCurrentDefinition&&<span className="task-current-definition-badge">依目前設定補登</span>}</h3><p>{item.status==="completed"?`已獲得 ${item.rewardStarsSnapshot} ⭐`:`完成可得 ${item.rewardStarsSnapshot} ⭐`}</p><small>{item.date!==todayKey&&`${formatTaipeiDate(item.date)}・`}{statusText}{backfilled&&<span className="task-backfilled-badge">補登</span>}{item.completedAt&&`・完成時間 ${formatTaipeiTime(item.completedAt)}`}{item.requestedAt&&item.status==="pending_approval"&&`・送出時間 ${formatTaipeiTime(item.requestedAt)}`}</small></div></div><div className="task-card-actions">
                {role==="孩子"&&!isYesterday&&item.status==="pending"&&canOperateSelectedChild&&<button className="task-primary" disabled={locked} onClick={()=>setTaskConfirm(item)}>{busy?"送出中…":"完成任務"}</button>}
                {role==="孩子"&&!isYesterday&&item.status==="pending"&&!canOperateSelectedChild&&<button disabled>僅可查看</button>}
                {role==="孩子"&&isYesterday&&(item.status==="pending"||item.status==="pending_approval")&&<button disabled>請家長協助補登</button>}
                {role==="孩子"&&!isYesterday&&item.status==="pending_approval"&&<button disabled>⏳ 等待家長確認</button>}
                {role==="家長"&&isYesterday&&currentDefinitionCandidate&&<button className="task-primary" disabled={locked} onClick={()=>setCurrentDefinitionConfirm(item)}>{busy?"補登中…":"補登完成"}</button>}
                {role==="家長"&&isYesterday&&!currentDefinitionCandidate&&(item.status==="pending"||item.status==="pending_approval")&&<><button className="task-primary" disabled={locked} onClick={()=>setTaskConfirm(item)}>{busy?"補登中…":"補登完成"}</button>{item.status==="pending_approval"&&<button disabled={locked} onClick={()=>runTaskAction(item,"reject")}>退回</button>}<button disabled={locked} onClick={()=>runTaskAction(item,"skip")}>昨天不適用</button></>}
                {role==="家長"&&!isYesterday&&item.status==="pending"&&<><button className="task-primary" disabled={locked} onClick={()=>runTaskAction(item,"complete")}>標記完成</button><button disabled={locked} onClick={()=>runTaskAction(item,"skip")}>今日不適用</button></>}
                {role==="家長"&&!isYesterday&&item.status==="pending_approval"&&<><button className="task-primary" disabled={locked} onClick={()=>runTaskAction(item,"approve")}>確認完成</button><button disabled={locked} onClick={()=>runTaskAction(item,"reject")}>退回</button><button disabled={locked} onClick={()=>runTaskAction(item,"skip")}>今日不適用</button></>}
                {role==="家長"&&item.status==="completed"&&<button className="task-undo" disabled={locked} onClick={()=>confirm(`確定撤銷「${item.titleSnapshot}」？星星也會同步扣回。`)&&runTaskAction(item,"undo")}>撤銷完成</button>}
                {role==="家長"&&item.status==="skipped"&&<button disabled={locked} onClick={()=>runTaskAction(item,"restore")}>恢復為待完成</button>}
            </div>{isYesterday&&item.status==="pending"&&role==="家長"&&<small className="task-backfill-date-note">星星將記錄於 {selectedDate.replaceAll("-","/")}</small>}</article>;
        };
        const progressMessage=!selectedRecords.length?`${dayLabel}沒有安排每日任務`:!progress.total?`${dayLabel}任務皆標記為不適用`:`${progress.completed} / ${progress.total} 項完成`;
        return <div key={selectedDate} className={`task-challenge task-date-view${isYesterday?" is-yesterday":" is-today"}`}>
            <section className="task-challenge-head">
                <div className="task-date-heading">{!isYesterday&&<button type="button" className="task-date-arrow task-date-previous" aria-label="查看昨天任務" onClick={()=>{setTaskConfirm(null);setTaskDateView("yesterday")}}>‹</button>}<div className="task-challenge-title"><p className="eyebrow">DAILY MISSION</p><h2>{isYesterday?"📝":"🎯"} {child.name} 的{dayLabel}任務</h2><p>{formatTaipeiDate(selectedDate)}</p></div>{isYesterday&&<button type="button" className="task-date-arrow task-date-next" aria-label="返回今天任務" onClick={()=>{setTaskConfirm(null);setTaskDateView("today")}}>›</button>}</div>
                {isYesterday&&<div className="task-backfill-banner" role="note"><strong>📝 昨天任務補登模式</strong><span>目前正在查看昨天的任務。完成後，星星與任務狀態會記錄在 {selectedDate.replaceAll("-","/")}。</span></div>}
                {isYesterday&&role==="孩子"&&pending.length>0&&<div className="task-child-backfill-note">昨天尚有未處理任務，請家長協助補登。</div>}
                <div className="task-summary-progress"><div className="task-summary-progress-copy"><div><span>{dayLabel}進度</span><strong>{progressMessage}</strong></div>{progress.percentage!==null&&<b>{progress.percentage}%</b>}</div>{progress.percentage!==null&&<div className="task-progress-track" role="progressbar" aria-label={`${child.name} 的${dayLabel}任務完成率`} aria-valuemin={0} aria-valuemax={100} aria-valuenow={progress.percentage}><i style={{width:`${progress.percentage}%`}}/></div>}{!selectedRecords.length&&<small>{isYesterday&&currentBackfillCandidates.length?"昨天原本沒有排定任務；下方另有依目前設定可補登的任務。":hasDefinitions?`${dayLabel}沒有符合星期排程的啟用任務，不影響連續達標。`:"請家長到「家庭設定 → 每日任務設定」新增固定任務。"}</small>}</div>
                {taskSyncError&&<div className="task-sync-error" role="status"><span>{dayLabel}任務尚未同步，請檢查網路。</span><button type="button" disabled={Boolean(taskBusy)} onClick={()=>void syncDailyTasks(true)}>{taskBusy==="sync"?"同步中…":"重新同步"}</button></div>}
            </section>
            <div className="task-stat-grid"><article><span>✨ {dayLabel}獲得</span><strong className={net<0?"summary-deduct":""}>{net>0?"＋":""}{net} ⭐</strong><small>固定任務 ＋{fixedStars}・其他獎勵 ＋{otherStars}・扣星 −{deductStars}</small></article><article><span>🔥 連續達標</span><strong>{streak} 天</strong><small>{!selectedRecords.length?`${dayLabel}沒有安排任務，不影響連續紀錄`:!progress.total?`${dayLabel}的任務皆不適用，不影響連續紀錄`:goal.met?`${dayLabel}已達標，目前連續 ${streak} 天！`:`再完成 ${remaining} 項即可達標`}</small></article><article><span>📈 本週完成率</span><strong>{weekly.percentage===null?"—":`${weekly.percentage}%`}</strong><small>{weekly.total?`${weekly.completed} / ${weekly.total} 項（週一至今天）`:"本週尚無有效任務"}</small><div className="task-mini-progress"><i style={{width:`${weekly.percentage??0}%`}}/></div></article></div>
            {!isYesterday&&approvalBacklog.length>0&&<section><Title text="待家長確認（含過往）"/><div className="task-card-grid">{approvalBacklog.map(renderTaskCard)}</div></section>}
            {isYesterday?<>
                <section><Title text="昨天原本任務"/>{selectedRecords.length?(pending.length?<div className="task-card-grid">{pending.map(renderTaskCard)}</div>:<p className="empty">昨天原本任務都已處理完成。</p>):<p className="empty">昨天原本沒有排定任務。</p>}</section>
                {currentBackfillCandidates.length>0&&<section className="task-current-definition-section"><Title text="可補登任務"/><p>以下任務依目前最新設定符合昨天日期，但不是昨天原本的任務快照。</p><div className="task-card-grid">{currentBackfillCandidates.map(renderTaskCard)}</div></section>}
                {selectedRecords.length>0&&<section><Title text="昨天已完成／不適用"/>{finished.length?<div className="task-card-grid">{finished.map(renderTaskCard)}</div>:<p className="empty">完成任務後會顯示在這裡</p>}</section>}
            </>:!selectedRecords.length?<section className="task-empty"><div>🎯</div><h2>{hasDefinitions?"今天沒有安排每日任務":"尚未建立每日任務"}</h2><p>{hasDefinitions?`${formatTaipeiDate(selectedDate)} 沒有符合排程的任務。`:"請家長到「家庭設定 → 每日任務設定」新增固定任務。"}</p>{!hasDefinitions&&role==="家長"&&<button className="primary" onClick={()=>goTab("家庭設定")}>前往新增每日任務</button>}</section>:<><section><Title text="今天待完成"/>{pending.length?<div className="task-card-grid">{pending.map(renderTaskCard)}</div>:<p className="empty">今天的待辦任務都處理完成了！</p>}</section><section><Title text="今天已完成／不適用"/>{finished.length?<div className="task-card-grid">{finished.map(renderTaskCard)}</div>:<p className="empty">完成任務後會顯示在這裡</p>}</section></>}
            {currentDefinitionConfirm&&<div className="modal-back"><section className="modal quick-confirm-modal"><button className="close" aria-label="關閉補登確認" onClick={()=>setCurrentDefinitionConfirm(null)}>×</button><h2>補登昨天任務？</h2><p className="task-current-definition-warning">此任務並非昨天原本排定，是依目前最新設定補登到昨天。</p><div className="confirm-box"><p>任務<strong>{currentDefinitionConfirm.iconSnapshot} {currentDefinitionConfirm.titleSnapshot}</strong></p><p>獎勵<strong>＋{currentDefinitionConfirm.rewardStarsSnapshot} ⭐</strong></p><p>記錄對象<strong>{child.name}</strong></p><p>紀錄日期<strong>{currentDefinitionConfirm.date.replaceAll("-","/")}</strong></p></div><div className="record-actions"><button className="save" disabled={Boolean(taskBusy)} onClick={()=>{const target=currentDefinitionConfirm;setCurrentDefinitionConfirm(null);void runTaskAction(target,"backfill_current_definition")}}>確認補登</button><button type="button" className="cancel-action" onClick={()=>setCurrentDefinitionConfirm(null)}>取消</button></div></section></div>}
        </div>;
    }
    ...
    function SettingsContent() {
        const c=child,allTasks=[...data.dailyTasks].sort((a,b)=>compareDailyTaskDefinitions(a,b,data.dailyTaskSortMode)),tasks=allTasks.filter(task=>task.habitStatus!=="graduated"),graduatedTasks=buildGraduatedHabitMetrics({childId:cid,todayKey,definitions:data.dailyTasks,records:data.dailyTaskRecords}),taskSettings=taskSettingsForChild(data.dailyTaskSettings,cid);
        ...
        <section className="settings-card wide daily-task-settings">
            ...
            {tasks.length?<div className="daily-task-settings-list">{tasks.map((task,index)=>{
                ...
                return <article id={`daily-task-setting-${task.id}`} className={`daily-task-settings-card${task.enabled?"":" is-disabled"}${missingChild||missingWeekday?" has-error":""}`} key={task.id}>
            ...
            {graduatedTasks.length>0&&<section className="settings-graduated-habits"><header><div><h3>🎓 已養成習慣</h3><p>不再每日排定與發放星星；重新啟用後會從今天開始重新累積。</p></div><span>{graduatedTasks.length}</span></header>{graduatedTasks.map(habit=><article key={habit.definitionId}><span>{habit.icon}</span><div><strong>{habit.title}</strong><small>{habit.graduatedAt?`${habit.graduatedAt.slice(0,10).replaceAll("-","/")} 畢業`:"已畢業"}・歷史完成 {habit.totalCompleted} 次</small></div><button type="button" disabled={Boolean(taskBusy)} onClick={()=>void runHabitAction(habit.definitionId,"reactivate")}>重新啟用</button></article>)}</section>}
