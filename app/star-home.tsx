...
export default function App() {
    ...
    async function saveAllSettings(){if(savingSettingsRef.current||imageUploading)return false;if(!hasUnsavedChanges)return true;const invalidTask=data.dailyTasks.find(task=>task.enabled&&!task.applicableChildIds.length);if(invalidTask){say(`「${invalidTask.title}」請至少選擇一位適用孩子`);return false}savingSettingsRef.current=true;setSavingSettings(true);const draft=clonePersistedState(data),next=prepareSettingsForSave(draft);try{return await persist(next,{preserveDraftOnFailure:true,optimistic:false,successMessage:"✅ 已儲存",draftForRebase:draft})}finally{savingSettingsRef.current=false;setSavingSettings(false)}}
    ...
    function Settings() {
        const c=child,tasks=[...data.dailyTasks].sort((a,b)=>a.sortOrder-b.sortOrder),taskSettings=taskSettingsForChild(data.dailyTaskSettings,cid);
        ...
        <section className="settings-card wide daily-task-settings">
            <div className="daily-task-settings-head"><div><h2>📋 每日任務設定</h2><p>建立一次任務，再選擇適用的孩子。每位孩子的完成狀態與星星獎勵會分開計算。</p></div></div>
            {tasks.length?<div className="daily-task-settings-list">{tasks.map((task,index)=>{
                ...
                return <article className={`daily-task-settings-card${task.enabled?"":" is-disabled"}${missingChild?" has-error":""}`} key={task.id}>
                    <h3>📋 每日任務 {String(index+1).padStart(2,"0")}</h3>
                    <div className="daily-task-settings-fields">
                        ...
                        <label>任務圖示<select value={task.icon} onChange={e=>updateTask(task.id,{icon:e.target.value})}>{BUILTIN_TASK_ICONS.map(icon=><option value={icon.value} key={icon.value}>{icon.value} {icon.name}</option>)}</select></label>
                        <label>完成獎勵<div className="task-reward-input"><input type="number" min="1" value={task.rewardStars} onChange={e=>updateTask(task.id,{rewardStars:positiveInt(e.target.value)})}/><span>顆星</span></div></label>
