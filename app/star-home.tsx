...
export default function App() {
    ...
    <section className="settings-card wide"><h2>🎁 獎品池</h2><div className="table-head"><span>圖示／圖片</span><span>獎品名稱</span><span>星星</span><span>操作</span></div><div className="edit-list">{data.rewards.map(r => <div key={r.id}><label className="reward-upload">{r.image?<img src={r.image} alt=""/>:r.icon} 📷<input hidden type="file" accept="image/*" onChange={e=>e.target.files?.[0]&&uploadReward(e.target.files[0],r.id)}/></label><input value={r.name} onChange={e => setData({ ...data, rewards: data.rewards.map(x => x.id === r.id ? { ...x, name: e.target.value } : x) })}/><input type="number" value={r.cost} onChange={e => setData({ ...data, rewards: data.rewards.map(x => x.id === r.id ? { ...x, cost: +e.target.value } : x) })}/><button className="delete" onClick={()=>confirm(`確定刪除「${r.name}」？`)&&setData({...data,rewards:data.rewards.filter(x=>x.id!==r.id)})}>刪除</button></div>)}</div><button className="add-line" onClick={() => setData({ ...data, rewards: [...data.rewards, { id: crypto.randomUUID(), icon: "🎁", name: "新獎品", cost: 10, stock: 0 }] })}>＋ 新增獎品</button></section><button className="settings-save" onClick={() => persist(data)}>儲存所有設定</button></div>; }
}
function Avatar({ c }: {
    c: Child;
